...
public final class GLCompat {
    ...
    public static void glScalef(float x, float y, float z) {
        if (mode == GL11.GL_MODELVIEW) {
            ...
            PoseStack.Pose entry = textureStack.last();
            entry.pose().scale(x, y, z);
        }
    }
    ...
    public static void glRotatef(float deg, float x, float y, float z) {
        if (mode == GL11.GL_MODELVIEW) {
            // 1.21.1: Axis.ofに相当するメソッドがないため、直接Quaternionfを使用
            ...
            Matrix4f model = textureStack.last().pose();
            model.rotate(new org.joml.Quaternionf().rotationAxis((float)Math.toRadians(deg), x, y, z));
        }
    }
    ...
    public static void glCallList(int i) {
        for (ModelBoxBase boxBase : modelRenderer.cubeList) {
            boxBase.render(ModelRenderer.poseStack, ModelRenderer.buffer, ModelRenderer.light, ModelRenderer.overlay, ModelRenderer.red, ModelRenderer.green, ModelRenderer.blue, ModelRenderer.alpha, modelRenderer.scale);
        }
    }
    ...
    private static void combine() {
        if (tex != null && pos != null) {
            ...
            if (vertexPrev2 != null) {
                ...
                quad.draw(ModelRenderer.poseStack, ModelRenderer.buffer, ModelRenderer.light, ModelRenderer.overlay, ModelRenderer.red, ModelRenderer.green, ModelRenderer.blue, ModelRenderer.alpha, 1F);
            }
        }
    }
