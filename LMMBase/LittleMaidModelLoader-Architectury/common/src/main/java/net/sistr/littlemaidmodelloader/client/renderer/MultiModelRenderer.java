...
// - setupTransforms → setupRotations (引数にscale追加)
// - hasLabel → shouldShowName
// - getHandSwingProgress → getAttackAnim
// - preferredHand → swingingArm
// - hasVehicle → isPassenger
// - isSneaking → isCrouching
// - entity.age → entity.tickCount
// - getTexture → getTextureLocation
// - dispatcher.targetedEntity → getDispatcher().targetedEntity (または entity == crosshairPickEntity)
...
public class MultiModelRenderer<T extends LivingEntity & IHasMultiModel> extends LivingEntityRenderer<T, MultiModel<T>> {
    private static final ResourceLocation NULL_TEXTURE = ResourceLocation.fromNamespaceAndPath(LMMLMod.MODID, "null");
    ...
    public void syncCaps(T entity, ModelMultiBase model, float partialTicks) {
        // 1.21.1: getHandSwingProgress → getAttackAnim
