...
import net.sistr.littlemaidrebirth.entity.targeting.TargetingSystem.Maid;
...
public class LMTargetGoal extends Goal {
    ...
    private boolean targeting() {
        ...
        if (TargetingSystem.needsEvacuation(maidWrapper, enemies, targetTagManager)) {
            TargetingSystem.getDangerousEnemies(maidWrapper, enemies, targetTagManager)
                    .forEach(mob -> this.maid.addFleeEntity(mob.getMob(), e -> !e.isAlive()
                            || this.maid.distanceToSqr(e) > (TargetingConfig.getDangerousAvoidDistance() + 4)
    ...
    public boolean canContinueToUse() {
        ...
        if (adjustedTickDelay(this.maid.getLastHurtByMobTimestamp()) == adjustedTickDelay(this.maid.tickCount - 1)) {
            return targeting();
        }
        // 現在のターゲットがまだ有効かチェック
    ...
    private List<Mob> getAroundMobs() {
        ...
        return this.maid.level().getEntitiesOfClass(
                Mob.class,
                this.maid.getBoundingBox().inflate(distance, distance / 2f, distance).inflate(1),
                mob -> mob != this.maid
    ...
    private boolean isTargetable(Mob mob, float distance) {
        return this.maid.distanceToSqr(mob) <= distance * distance
    ...
    private List<LittleMaidEntity> getAroundMaids() {
        ...
        return this.maid.level().getEntitiesOfClass(
                ...
                this.maid.getBoundingBox()
                        .inflate(distance, distance / 2f, distance).inflate(1),
                maid -> maid != this.maid);
    }
}
