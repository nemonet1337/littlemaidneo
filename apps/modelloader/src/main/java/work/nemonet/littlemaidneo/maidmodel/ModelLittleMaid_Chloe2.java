...
/**
 * 旧 GLCompat 版 ModelLittleMaid_Chloe2 の忠実移植。
 * 二節腕・太もも付き二節脚・多段スカート裾・ポニーテールを含む（Beverly7 より一回り小柄）。
...
public class ModelLittleMaid_Chloe2 extends LMModel<MultiModelRenderState> {
    ...
    private static void buildMesh(MeshDefinition mesh, CubeDeformation deform, float pyoffset) {
        ...
        var bipedBody = bipedTorso.addOrReplaceChild("biped_body", CubeListBuilder.create()
                .texOffs(20, 26).addBox(-3, -7.5F, -2.1F, 6, 8, 4, deform)
                .texOffs(24, 16).addBox(-1, -8.8F, -1, 2, 2, 2, deform.extend(0.5F)),
                PartPose.offsetAndRotation(0, 0, 0, -0.1F, 0, 0));
        bipedBody.addOrReplaceChild("breast_right", CubeListBuilder.create()
                .texOffs(20, 20).addBox(-3, 0, -2, 3, 2, 3, deform.extend(0.1F)),
                PartPose.offsetAndRotation(-0.5F, -6.2F, -2.1F, 0.785F, 0, -0.15F));
        bipedBody.addOrReplaceChild("breast_left", CubeListBuilder.create()
                .texOffs(32, 20).mirror().addBox(0, 0, -2, 3, 2, 3, deform.extend(0.1F)),
                PartPose.offsetAndRotation(0.5F, -6.2F, -2.1F, 0.785F, 0, 0.15F));
        ...
        var bipedRightArm = bipedNeck.addOrReplaceChild("biped_right_arm", CubeListBuilder.create()
                .texOffs(0, 16).addBox(-1, -1, -1, 2, 5, 3, deform),
                PartPose.offset(-4, -6.5F, 0));
