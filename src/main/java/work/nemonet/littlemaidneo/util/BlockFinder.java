...
public class BlockFinder {
        ...
        for (int i = 0; i < range; i++) {
            for (BlockPos checkedPos : checked) {
                for (Direction direction : Direction.values()) {
                    int num = checkedPos.get(direction.getAxis())
                            - centerPos.get(direction.getAxis());
                    //0の場合は両方向にやる
                    if (0 <= num) {
                        BlockPos checkPos = checkedPos.relative(direction);
                    ...
                    if (num <= 0) {
                        BlockPos checkPos = checkedPos.relative(direction.getOpposite());
                        if (target.test(checkPos))
                            return Optional.of(checkPos);
                        if (linkable.test(checkPos))
                            nowChecked.add(checkPos);
    ...
    public static Optional<BlockPos> findHorizonPos(Level world, BlockPos centerPos, int layer, int horizon, Predicate<BlockPos> target) {
        ...
        for (int l = 0; l < layer; l++) {
            BlockPos checkPos = centerPos.relative(layer % 2 == 0 ? Direction.UP : Direction.DOWN, layer >> 1);
    ...
    public static Optional<BlockPos> findHorizon(Level world, Set<BlockPos> prevSearched, Set<BlockPos> allSearched,
    ...
    public static boolean isAir(Level world, BlockPos pos, Direction dir) {
        return world.isEmptyBlock(pos.relative(dir));
    }
